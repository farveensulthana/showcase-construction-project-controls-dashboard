...
public class RiskService : IRiskService
{
    ...
    public async Task<RiskDto> CreateRiskAsync(RiskCreateDto dto, CancellationToken ct = default)
    {
        ...
        await _risks.AddAsync(entity, ct);
        await _risks.SaveChangesAsync(ct);
        project = await _projects.GetByIdAsync(entity.ProjectId, ct);
        return MapRisk(entity, project?.Code);
    }

    public async Task<RiskDto?> UpdateRiskAsync(int id, RiskUpdateDto dto, CancellationToken ct = default)
    {
        var entity = await _risks.Query().FirstOrDefaultAsync(r => r.Id == id, ct);
        ...
        entity.ImpactType = dto.ImpactType;
        entity.ImpactDescription = dto.ImpactDescription;
        entity.ImpactCost = dto.ImpactCost;
        ...
        _risks.Update(entity);
        await _risks.SaveChangesAsync(ct);
        var project = await _projects.GetByIdAsync(entity.ProjectId, ct);
        return MapRisk(entity, project?.Code);
    }

    public async Task<bool> DeleteRiskAsync(int id, CancellationToken ct = default)
    {
        var entity = await _risks.Query().FirstOrDefaultAsync(r => r.Id == id, ct);
        if (entity is null) return false;
        _risks.Delete(entity);
        await _risks.SaveChangesAsync(ct);
        return true;
    }
