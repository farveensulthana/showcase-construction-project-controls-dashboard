...
public partial class Risks : ComponentBase
{
    ...
    private List<RiskDto> PagedRisks => FilteredRisks.Skip((_page - 1) * PageSize).Take(PageSize).ToList();
    ...
    private void OnRowKeydown(KeyboardEventArgs e, RiskDto r) => KeyboardActivation.OnActivateKey(e, () => _selectedRisk = r);
