...
export class Risks implements OnInit {
  ...
  readonly matrixClass = matrixClass;
  ...
  matrixLookup = computed(() => {
    const map = new Map<string, RiskMatrixCellViewModel>();
    this.matrix().forEach((cell) => map.set(`${cell.probability}-${cell.severity}`, cell));
    return map;
  });

  constructor(private risksApi: RisksService, private riskMatrixApi: RiskMatrixService, private router: Router) {}
  ...
  readonly reversedProbabilities = [...probabilities].reverse();

  matrixCell(probability: RiskProbability, severity: RiskSeverity): RiskMatrixCellViewModel | undefined {
    return this.matrixLookup().get(`${probability}-${severity}`);
  }

  matrixCellDisplay(probability: RiskProbability, severity: RiskSeverity): string {
    const cell = this.matrixCell(probability, severity);
    if (!cell) return '';
    const ids = cell.riskIds.slice(0, 2);
    const overflow = cell.count > ids.length ? ` +${cell.count - ids.length}` : '';
    return ids.join(', ') + overflow;
  }

  matrixTone(severity: RiskSeverity, probability: RiskProbability): string {
    return matrixTone(severity, probability);
  }
  ...
  onRowKeydown(event: KeyboardEvent, r: RiskSummaryDto): void {
    onActivateKey(event, () => this.selectedRisk.set(r));
  }
